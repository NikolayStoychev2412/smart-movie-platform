...
function CastCarousel({ cast, theme, language }: { cast: CastMember[]; theme: string; language: string }) {
  ...
  if (!cast?.length) return <div className={`text-center py-12 rounded-lg ${theme === "dark" ? "bg-gray-900 text-gray-500" : "bg-gray-100 text-gray-400"}`}><User className="w-16 h-16 mx-auto mb-3 opacity-50" /><p>{language === "bg" ? "Няма информация" : "No cast info"}</p></div>;

  return (
    <div className="relative">
      {canScrollLeft && <button onClick={() => scroll("left")} className={`absolute left-0 top-1/2 -translate-y-1/2 z-20 p-2 rounded-full shadow-xl ${theme === "dark" ? "bg-gray-800 text-white" : "bg-white text-gray-800 border"}`} style={{marginTop:"-20px"}}><ChevronLeft className="w-6 h-6" /></button>}
      {canScrollRight && <button onClick={() => scroll("right")} className={`absolute right-0 top-1/2 -translate-y-1/2 z-20 p-2 rounded-full shadow-xl ${theme === "dark" ? "bg-gray-800 text-white" : "bg-white text-gray-800 border"}`} style={{marginTop:"-20px"}}><ChevronRight className="w-6 h-6" /></button>}
      <div ref={scrollRef} className="flex gap-4 pb-4" style={{ overflowX: "auto", scrollbarWidth: "none", scrollSnapType: "x mandatory" }}>
        {[...cast].sort((a,b) => (a.order??999)-(b.order??999)).slice(0,15).map((actor, i) => (
          <div key={actor.id||i} className={`flex-shrink-0 w-[150px] rounded-lg overflow-hidden shadow-lg hover:scale-105 transition-transform ${theme === "dark" ? "bg-gray-900" : "bg-white border"}`} style={{scrollSnapAlign:"start"}}>
            <div className="aspect-[2/3]">{actor.profile_path ? <img src={`https://image.tmdb.org/t/p/w185${actor.profile_path}`} alt={actor.name} className="w-full h-full object-cover" /> : <div className={`w-full h-full flex items-center justify-center ${theme === "dark" ? "bg-gray-800" : "bg-gray-200"}`}><User className="w-16 h-16 text-gray-500" /></div>}</div>
            <div className="p-3"><p className={`font-semibold text-sm truncate ${theme === "dark" ? "text-white" : "text-gray-900"}`}>{actor.name}</p>{actor.character && <p className="text-xs mt-1 truncate text-gray-400">{actor.character}</p>}</div>
          </div>
        ))}
      </div>
    </div>
...
function FactsPanel({ movie, theme, language }: { movie: MovieDetail; theme: string; language: string }) {
  ...
  return (
    <div className={`rounded-lg p-5 ${theme === "dark" ? "bg-gray-900" : "bg-white border shadow-sm"}`}>
      <h3 className={`font-bold text-lg mb-4 ${theme === "dark" ? "text-white" : "text-gray-900"}`}>{language === "bg" ? "Информация" : "Facts"}</h3>
      <div className="space-y-4">
        {director && <div><p className="text-sm font-medium text-gray-400">{language==="bg"?"Режисьор":"Director"}</p><p className={`font-medium mt-0.5 ${theme==="dark"?"text-white":"text-gray-900"}`}>{director}</p></div>}
        {writers?.length > 0 && <div><p className="text-sm font-medium text-gray-400">{language==="bg"?"Сценарист":"Writer"}</p><p className={`font-medium mt-0.5 ${theme==="dark"?"text-white":"text-gray-900"}`}>{writers.map(w=>w.name).join(", ")}</p></div>}
        {facts.map((f,i) => f.value && <div key={i}><p className="text-sm font-medium text-gray-400">{f.label}</p><p className={`font-medium mt-0.5 ${theme==="dark"?"text-white":"text-gray-900"}`}>{f.value}</p></div>)}
        {movie.production_companies?.length > 0 && <div><p className="text-sm font-medium mb-2 text-gray-400">{language==="bg"?"Продукция":"Production"}</p><div className="space-y-2">{movie.production_companies.slice(0,3).map((c,i) => <div key={c.id||i} className="flex items-center gap-2">{c.logo_path ? <img src={`https://image.tmdb.org/t/p/w92${c.logo_path}`} alt={c.name} className={`h-6 w-auto object-contain ${theme==="dark"?"":"brightness-0"}`}/> : <Building2 className="w-5 h-5 text-gray-500"/>}<span className={`text-sm ${theme==="dark"?"text-gray-300":"text-gray-700"}`}>{c.name}</span></div>)}</div></div>}
        {movie.homepage && <a href={movie.homepage} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-tmdb-light-blue hover:underline text-sm mt-4"><ExternalLink className="w-4 h-4"/>{language==="bg"?"Официален сайт":"Official Website"}</a>}
      </div>
    </div>
  );
}
...
function VideosSection({ movie, theme, language }: { movie: MovieDetail; theme: string; language: string }) {
  ...
  return (
    <section>
      <h2 className={`text-2xl font-bold mb-5 ${theme==="dark"?"text-white":"text-gray-900"}`}>{language==="bg"?"Видео":"Videos"} <span className="ml-2 text-base font-normal text-gray-500">({videos.length})</span></h2>
      {playingVideo && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 p-4" onClick={() => setPlayingVideo(null)}><button onClick={() => setPlayingVideo(null)} className="absolute top-4 right-4 p-2 text-white hover:text-gray-300"><X className="w-8 h-8"/></button><div className="w-full max-w-5xl aspect-video" onClick={e => e.stopPropagation()}><iframe src={`https://www.youtube.com/embed/${playingVideo}?autoplay=1`} title="Video" className="w-full h-full rounded-lg" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen/></div></div>}
      <div className="relative">
        {canScrollLeft && <button onClick={() => scroll("left")} className={`absolute left-0 top-1/2 -translate-y-1/2 z-20 p-2 rounded-full shadow-xl ${theme==="dark"?"bg-gray-800 text-white":"bg-white text-gray-800 border"}`}><ChevronLeft className="w-6 h-6"/></button>}
        {canScrollRight && <button onClick={() => scroll("right")} className={`absolute right-0 top-1/2 -translate-y-1/2 z-20 p-2 rounded-full shadow-xl ${theme==="dark"?"bg-gray-800 text-white":"bg-white text-gray-800 border"}`}><ChevronRight className="w-6 h-6"/></button>}
        <div ref={scrollRef} className="flex gap-4 pb-4" style={{ overflowX: "auto", scrollbarWidth: "none", scrollSnapType: "x mandatory" }}>
          {videos.map(v => (
...
function SimilarMoviesSection({ movies, theme, language }: { movies: Movie[]; theme: string; language: string }) {
  ...
  const checkScroll = () => { if (scrollRef.current) { const { scrollLeft, scrollWidth, clientWidth } = scrollRef.current; setCanScrollLeft(scrollLeft > 5); setCanScrollRight(scrollLeft < scrollWidth - clientWidth - 5); } };
  const scroll = (d: "left"|"right") => { scrollRef.current?.scrollBy({ left: d==="left" ? -320 : 320, behavior: "smooth" }); setTimeout(checkScroll, 350); };
  useEffect(() => { checkScroll(); const ref = scrollRef.current; ref?.addEventListener("scroll", checkScroll); return () => ref?.removeEventListener("scroll", checkScroll); }, [movies]);
  if (!movies?.length) return null;

  return (
    <section className="relative">
      <h2 className={`text-2xl font-bold mb-5 ${theme==="dark"?"text-white":"text-gray-900"}`}>{language==="bg"?"Подобни филми":"Similar Movies"}</h2>
      {canScrollLeft && <button onClick={() => scroll("left")} className={`absolute left-0 top-1/2 z-20 p-2 rounded-full shadow-xl ${theme==="dark"?"bg-gray-800 text-white":"bg-white text-gray-800 border"}`} style={{marginTop:"20px"}}><ChevronLeft className="w-6 h-6"/></button>}
      {canScrollRight && <button onClick={() => scroll("right")} className={`absolute right-0 top-1/2 z-20 p-2 rounded-full shadow-xl ${theme==="dark"?"bg-gray-800 text-white":"bg-white text-gray-800 border"}`} style={{marginTop:"20px"}}><ChevronRight className="w-6 h-6"/></button>}
      <div ref={scrollRef} className="flex gap-4 pb-4" style={{ overflowX: "auto", scrollbarWidth: "none", scrollSnapType: "x mandatory" }}>
        {movies.map((m, i) => {
...
export default function MovieDetails() {
  ...
  const getBtn = () => {
    if (!watchlistStatus) return { icon: <Plus className="w-5 h-5"/>, text: language==="bg"?"Добави":"Add to List", cls: "bg-tmdb-dark-blue/80 text-gray-300 hover:text-white" };
    ...
    return map[watchlistStatus] || { icon: <Plus className="w-5 h-5"/>, text: "Add", cls: "bg-tmdb-dark-blue/80 text-gray-300" };
  };
  const btn = getBtn();

  if (loading) return <div className={`min-h-screen ${theme==="dark"?"bg-tmdb-dark":"bg-gray-50"}`}><div className="animate-pulse"><div className={`h-[500px] ${theme==="dark"?"bg-gray-800":"bg-gray-300"}`}/></div></div>;
  if (error || !movie) return <div className={`min-h-screen flex items-center justify-center ${theme==="dark"?"bg-tmdb-dark":"bg-gray-50"}`}><div className="text-center"><Film className="w-20 h-20 mx-auto mb-4 text-gray-500"/><p className="text-xl text-gray-400">{error||"Not found"}</p><button onClick={() => navigate(-1)} className="mt-4 px-6 py-2 bg-tmdb-light-blue text-tmdb-dark-blue rounded-lg font-medium">{language==="bg"?"Назад":"Back"}</button></div></div>;
                ...
                <div className="relative">
                  ...
                  {showWatchlistMenu && <>
                    ...
                    <div className={`absolute top-full left-0 mt-2 w-48 rounded-lg shadow-xl z-50 overflow-hidden ${theme==="dark"?"bg-gray-800":"bg-white border"}`}>
                      ...
                      {watchlistStatus && <><div className={`border-t ${theme==="dark"?"border-gray-700":"border-gray-200"}`}/><button onClick={removeFromWatchlist} className="w-full flex items-center gap-3 px-4 py-3 text-left text-red-500 hover:bg-red-500/10"><X className="w-4 h-4"/>{language==="bg"?"Премахни":"Remove"}</button></>}
                    </div>
                  </>}
                </div>
